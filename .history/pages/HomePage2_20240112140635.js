...
const HomePage2 = ({ navigation }) => {
    ...
    const [selectedFrom, setSelectedFrom] = useState("City1");  // Default selected value for "From"
    const [selectedTo, setSelectedTo] = useState("City2");  // Default selected value for "To"

    const [date, setDate] = useState(new Date())
    const [open, setOpen] = useState(false)

    const formatDate = (date) => {
        return date.toLocaleDateString(); // Get only the date without time
    };
